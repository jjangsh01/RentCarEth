...
const OwnerPage = () => {
    ...
    const loadMyCars = async () => {
        ...
        try {
            const contract = new ethers.Contract(import.meta.env.VITE_CONTRACT_REGISTRY, CarRegistryABI.abi, signer);
            const plates = await contract.getCarPlates();
            const result = [];
            for (const plate of plates) {
                const [id, model, location, price, status, renter, owner] = await contract.getCar(plate);
    ...
    if (loading) return <div className="min-h-screen flex items-center justify-center bg-slate-900 text-white text-xl">🔄 MetaMask 연결 중...</div>;
                ...
                <div className="mb-8 text-center">
                    <div className="inline-flex items-center gap-4 bg-gradient-to-r from-white/10 to-white/5 backdrop-blur-xl px-8 py-4 rounded-3xl border border-white/20 shadow-2xl">
                        ...
                        <div className="text-left">
                            <h1 className="text-2xl font-bold bg-gradient-to-r from-white to-purple-200 bg-clip-text text-transparent">차량 소유자 대시보드</h1>
                            ...
                            <div className="p-6 space-y-3">
                                ...
                                <button
                                    className={`w-full group flex items-center gap-4 py-4 px-4 rounded-2xl font-medium transition-all duration-300 ${
                                        selectedTab === "add" ? "bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-lg shadow-purple-500/30 scale-105" : "text-purple-200 hover:bg-white/10 hover:text-white hover:scale-102"
                                    }`}
                                    onClick={() => setSelectedTab("add")}
                                >
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-md transition-all ${selectedTab === "add" ? "bg-white/20" : "bg-gradient-to-br from-green-500 to-emerald-600 group-hover:scale-110"}`}>
                                    ...
                                    {selectedTab === "add" && <div className="w-2 h-2 bg-white rounded-full animate-pulse"></div>}
                                </button>

                                <button
                                    className={`w-full group flex items-center gap-4 py-4 px-4 rounded-2xl font-medium transition-all duration-300 ${
                                        selectedTab === "list" ? "bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-lg shadow-purple-500/30 scale-105" : "text-purple-200 hover:bg-white/10 hover:text-white hover:scale-102"
                                    }`}
                                    onClick={() => setSelectedTab("list")}
                                >
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-md transition-all ${selectedTab === "list" ? "bg-white/20" : "bg-gradient-to-br from-blue-500 to-purple-600 group-hover:scale-110"}`}>
                                        <span className="text-lg">🚘</span>
                                    </div>
                                    <div className="flex-1 text-left">
                                    ...
                                    {totalCars > 0 && <div className="bg-green-500 text-white text-xs px-2 py-1 rounded-full font-bold">{totalCars}</div>}
                                    {selectedTab === "list" && <div className="w-2 h-2 bg-white rounded-full animate-pulse"></div>}
                                </button>

                                <button
                                    className={`w-full group flex items-center gap-4 py-4 px-4 rounded-2xl font-medium transition-all duration-300 ${
                                        selectedTab === "rented" ? "bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-lg shadow-purple-500/30 scale-105" : "text-purple-200 hover:bg-white/10 hover:text-white hover:scale-102"
                                    }`}
                                    onClick={() => setSelectedTab("rented")}
                                >
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-md transition-all ${selectedTab === "rented" ? "bg-white/20" : "bg-gradient-to-br from-yellow-500 to-orange-600 group-hover:scale-110"}`}>
                                        <span className="text-xl"></span>
                                    </div>
                                    <div className="flex-1 text-left">
                                    ...
                                    {rentedCars > 0 && <div className="bg-yellow-500 text-black text-xs px-2 py-1 rounded-full font-bold animate-pulse">{rentedCars}</div>}
                                    {selectedTab === "rented" && <div className="w-2 h-2 bg-white rounded-full animate-pulse"></div>}
                                </button>
                            </div>
                        ...
                        <div className="bg-gradient-to-br from-white/10 via-white/5 to-transparent backdrop-blur-2xl rounded-3xl border border-white/20 shadow-2xl min-h-[600px] overflow-hidden">
                            ...
                            <div className="bg-gradient-to-r from-purple-600/10 via-pink-500/10 to-blue-500/10 p-6 border-b border-white/10">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 bg-gradient-to-br from-purple-500 to-pink-500 rounded-xl flex items-center justify-center shadow-lg">
                                        <span className="text-xl">{selectedTab === "add" ? "➕" : selectedTab === "list" ? "🚘" : ""}</span>
                                    </div>
                                    <div>
                            ...
                            <div className="p-6">
                                <div className="animate-fadeIn">
                                    {selectedTab === "add" && <AddCarForm signer={signer} onCarUpdated={handleCarChange} />}
                                    {selectedTab === "list" && <MyCarManager signer={signer} account={account} refreshSignal={refreshSignal} onCarUpdated={handleCarChange} myCars={myCars} />}
                                    {selectedTab === "rented" && <MyRentedCars signer={signer} account={account} />}
                                </div>
                            </div>
                        </div>
                    </main>
